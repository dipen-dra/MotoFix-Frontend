...
const ProfilePage = ({ currentUser, setCurrentUser }) => {
    const [profile, setProfile] = useState({ 
        ...
        phone: '', 
        address: '', 
        profilePictureUrl: '', // FIX: Added to initial state
        latitude: '', 
        longitude: '',
    ...
    useEffect(() => { 
        const fetchWorkshopProfile = async () => {
            try {
                ...
                // Fallback state if fetching fails
                const fallbackState = {
                    ownerName: currentUser?.fullName || '',
                    workshopName: '', email: currentUser?.email || '', phone: '', address: '', 
                    profilePictureUrl: '', latitude: '', longitude: '', pickupDropoffAvailable: false, 
                    pickupDropoffCostPerKm: ''
                };
                setProfile(fallbackState);
            ...
            async (position) => {
                ...
                try {
                    ...
                    if (data && data.display_name) {
                        ...
                        toast.warn("Could not determine detailed address. Lat/Lon saved."); 
    ...
    const handleSave = async () => {
        const formData = new FormData();
        // Append all relevant fields from the profile state
        Object.keys(profile).forEach(key => {
            if (key === 'newProfilePicture') {
                formData.append('profilePicture', profile.newProfilePicture);
            } else if (key !== 'latitude' && key !== 'longitude' && key !== 'profilePictureUrl' && key !== '_id' && key !== 'location' && key !== '__v' && key !== 'createdAt' && key !== 'updatedAt' && key !== 'profilePicture') {
        ...
        try {
            ...
            // FIX: This correctly updates the parent component's state
            if (setCurrentUser) {
                setCurrentUser(prevUser => ({
                    ...prevUser, // Keep existing fields like role
                    ...data      // Overwrite with new data from API (ownerName, profilePictureUrl, etc.)
                }));
            }
    ...
    // FIX: Simplified image source logic using a single source of truth (profile.profilePictureUrl)
    const handleImageError = (e) => { e.target.onerror = null; e.target.src = `https://ui-avatars.com/api/?name=${profile.workshopName?.charAt(0) || 'A'}&background=e2e8f0&color=4a5568&size=128`; }
    const profilePictureSrc = profile.profilePictureUrl || `https://ui-avatars.com/api/?name=${profile.workshopName?.charAt(0) || 'A'}&background=e2e8f0&color=4a5568&size=128`;
                        ...
                        <div className="lg:col-span-2 space-y-4">
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-2">
                                    <textarea id="address" name="address" rows="3" value={profile.address || ''} onChange={handleChange} disabled={!isEditing || isFetchingLocation} className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-blue-500 focus:border-blue-500 dark:text-white disabled:bg-gray-200 dark:disabled:bg-gray-700/50" placeholder="Enter workshop address or fetch current location"></textarea>
                            ...
                            <div className="border border-gray-300 dark:border-gray-600 rounded-lg p-4 space-y-3 bg-gray-50 dark:bg-gray-700/50">
                                ...
                                <div>
                                    <input 
                                        ...
                                        checked={profile.pickupDropoffAvailable || false} 
...
const WorkshopApplicationsPage = () => {
    ...
    const [filterStatus, setFilterStatus] = useState('Pending'); // Default to showing only Pending

    const fetchApplications = async () => {
        setLoading(true);
        try {
            // Fetch all applications, filtering is done on frontend for now
    ...
    return (
        <div className="space-y-6">
            ...
            {/* END NEW: Filter/Section Selector */}

            {filteredApplications.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {filteredApplications.map(app => ( // Use filteredApplications here
                        <Card key={app._id}>
                            ...
                            <div className="space-y-2 text-sm text-gray-700 dark:text-gray-300 border-t pt-4 mt-4">
                                ...
                                {/* Corrected from app.message to app.notes */}
                                <p><strong>Notes:</strong> <span className="italic text-gray-500 dark:text-gray-400">{app.notes || 'No message provided.'}</span></p>
                            </div>

                            {/* Display Company Document Image */}
                            <div className="mt-4 border-t pt-4 border-gray-200 dark:border-gray-700">
                                <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-2">Company Document</h3>
                                {app.companyDocumentImage ? (
                                    <p className="text-gray-600 dark:text-gray-300">
                                        <a 
                                            href={`http://localhost:5050/${app.companyDocumentImage.replace(/\\/g, '/')}`} 
                                            target="_blank" 
                                            rel="noopener noreferrer" 
                                            className="text-blue-500 hover:underline flex items-center gap-1"
                                        >
                                            <Paperclip size={16} /> View Document (PDF)
                                        </a>
                                    </p>
                                ) : (
                                    <p className="text-gray-500 dark:text-gray-400 italic">No company document uploaded.</p>
                                )}
                            </div>
                            {/* End Company Document Image */}
                            ...
                            <div className="flex justify-end gap-3 mt-6">
                                ...
                                {/* Display status badge for non-pending applications */}
                                {app.status !== 'Pending' && (
                                    <span className={`px-4 py-1.5 text-sm font-medium rounded-full ${getStatusColor(app.status)}`}>
                                        {app.status}
                                    </span>
                                )}
                            </div>
                 ...
                 <Card className="text-center py-12">
                     ...
                     <h3 className="mt-4 text-xl font-semibold">No {filterStatus === 'All' ? '' : filterStatus} Applications Found</h3>
                     <p className="text-gray-500 dark:text-gray-400 mt-2">There are currently no {filterStatus.toLowerCase()} workshop applications to review in this section.</p>
                 </Card>
            )}
        </div>
    );
...
//     useEffect(() => {
//         const fetchInitialCount = async () => { if (!user) return; try { const response = await apiFetch('/chat/users'); const data = await response.json(); const unreadConversations = data.data.filter(c => c.unreadCount > 0); setTotalUnreadCount(unreadConversations.length); } catch (error) { console.error("Could not fetch initial unread count", error); } };
//         fetchInitialCount();
...
//         return () => { socket.off('new_message_notification', notificationListener); socket.off('messages_read_by_admin', readListener); };
//     }, [user]);
...
//                 const response = await apiFetch('/profile'); 
//                 const { data } = await response.json();
//                 setCurrentUser({ ...data, role: user.role });
//             } catch (error) { 
//                 console.error("Failed to fetch admin/workshop profile for header:", error);
//                 setCurrentUser({ ownerName: user?.fullName || 'Admin', workshopName: 'Profile Not Set', profilePictureUrl: '', role: user?.role || 'admin' });
...
//     useEffect(() => { const handleHashChange = () => { const path = window.location.hash.replace('#/admin/', '').split('?')[0]; const pageName = path.split('/')[0] || 'dashboard'; setActivePage(pageName); }; window.addEventListener('hashchange', handleHashChange); handleHashChange(); return () => window.removeEventListener('hashchange', handleHashChange); }, []);

//     const renderPage = () => {
//         if (!currentUser) { return <div className="text-center p-12">Loading Admin Data...</div>; }
//         const hash = window.location.hash.replace('#/admin/', '');
//         const [page, id] = hash.split('/');
//         switch (page) {
//             case 'dashboard': return <DashboardPage />;
//             case 'bookings': return id ? <BookingDetailsPage bookingId={id} /> : <BookingsPage />;
...
//             case 'chat': return <AdminChatPage currentUser={currentUser} />;
//             case 'workshops': return id ? <SuperadminWorkshopDetailsPage workshopId={id} /> : <WorkshopManagementPage />;
//             case 'applications': return <WorkshopApplicationsPage />;
//             default: window.location.hash = '#/admin/dashboard'; return <DashboardPage />;
//         }
//     };

//     const handleImageError = (e) => { e.target.onerror = null; e.target.src = `https://ui-avatars.com/api/?name=${currentUser?.ownerName?.charAt(0) || 'A'}&background=e2e8f0&color=4a5568&size=40`; }
//     const profilePictureSrc = currentUser?.profilePictureUrl || `https://ui-avatars.com/api/?name=${currentUser?.ownerName?.charAt(0) || 'A'}&background=e2e8f0&color=4a5568&size=40`;
